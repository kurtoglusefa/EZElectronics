...
class Authenticator {
     ...
     */
    isLoggedIn_user(req: any, res: any, next: any) {
     ...
     */
    isCustomer_user(req: any, res: any, next: any) {
        if (req.isAuthenticated() && Utility.isCustomer(req.user)) return 1
        return 0
    }
    isCustomer_check(req: any) {
        if (req.isAuthenticated() && Utility.isCustomer(req.user)) return 1
        return 0;
    }
     ...
     */
    isManager_user(req: any, res: any, next: any) {
    ...
    isManager_check(req: any) {
        if (req.isAuthenticated() && Utility.isManager(req.user)) return 1;
        return 0;
    }

    isAdmin_user(req: any, res: any, next: any) {
        if (req.isAuthenticated() && Utility.isAdmin(req.user)) return 1
        return 0
    }
