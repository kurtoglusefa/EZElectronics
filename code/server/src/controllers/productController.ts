...
class ProductController {
     ...
     */
    async registerProducts(model: string, category: string, arrivalDate: string | null,  sellingPrice: number, quantity: number, details: string | null): Promise<void> {
        await this.dao.addProduct(model, category, arrivalDate, sellingPrice, quantity, details)
     }
    ...
    async changeProductQuantity(model: string, newQuantity: number, changeDate: string | null): Promise<number> {
            const totalQuantity = await this.dao.changeProductQuantity(model, newQuantity, changeDate)
    ...
    async sellProduct(model: string, quantity: number, sellingDate: string | null): Promise<number> {
        const totalQuantity = await this.dao.sellProduct(model, quantity, sellingDate)
        return totalQuantity
     }
    ...
    async getProducts(grouping: string | null, category: string | null, model: string | null): Promise<Product[]> {
        return await this.dao.getProducts(grouping, category, model)
     }
    ...
    async getAvailableProducts(grouping: string | null, category: string | null, model: string | null):Promise<Product[]> {
        return await this.dao.getAvailableProducts(grouping, category, model)
     }
    ...
    async deleteAllProducts(): Promise <Boolean> {
        const result = await this.dao.deleteAllProducts()
        return result
     }
    ...
    async deleteProduct(model: string): Promise <Boolean>  {
        const result = await this.dao.deleteProduct(model)
        return result
     }
